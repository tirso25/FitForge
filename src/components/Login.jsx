...
export default function Login(props) {
    ...
    const validateData = (data) => {
        ...
        if (!validateEmailOrUsername(data.email)) {
            showErrorMessage('Invalid email or username format');
            return false;
        }
    ...
    const login = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                if (message.type === 'pending_activation') {
                    setStatus('idle');
                    ...
                    setTimeout(() => {
                        navigate(`/checkCode?e=${message.encryptedEmail}`);
                    }, 2000);
                    return;
                }
                ...
                <div className="rules-container" ref={rulesContainerRef}>
                    <div id="rules">
                        <p id="email_rule" ref={emailRuleRef}>
                            <b>Email or Username</b>: Valid email address or username (5–20 chars, lowercase, numbers, dots, underscores or hyphens, no double symbols, must start/end with letter or number)
                        </p>
                        <p id="pwd_rule" ref={pwdRuleRef}>
                            <b>Password</b>: Min. 8 characters, maximum 128, 1 uppercase letter, 1 lowercase letter, 1 number, and 1 symbol
                        </p>
                    </div>
